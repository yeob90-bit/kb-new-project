...
describe("Acceptance — Dashboard UI", () => {
  it("빈 상태에서 안내 문구를 보여준다", () => {
    ...
    expect(screen.getByTestId("empty-state")).toHaveTextContent(
      "파일을 업로드하거나 샘플 데이터로 먼저 확인해보세요.",
    );
  });

  it("샘플 분석 후 KPI가 Reference 수치와 일치한다", async () => {
    const user = userEvent.setup();
    renderDashboard();

    await user.click(screen.getByTestId("load-sample"));

    expect(screen.getByTestId("kpi-active")).toHaveTextContent("27");
    expect(screen.getByTestId("kpi-p1")).toHaveTextContent("2");
    expect(screen.getByTestId("kpi-remark")).toHaveTextContent("16");
  ...
  it("Active Queue 테이블과 Band 필터가 동작한다", async () => {
    ...
    expect(within(table).getByText("L0029")).toBeInTheDocument();
    expect(within(table).getByText("L0025")).toBeInTheDocument();
    ...
    expect(within(table).getByText("L0029")).toBeInTheDocument();
    expect(within(table).getByText("L0025")).toBeInTheDocument();
    expect(within(table).queryByText("L0031")).not.toBeInTheDocument();
    expect(
  ...
  it("검색 필터로 계좌를 좁힌다", async () => {
    ...
    await user.type(screen.getByTestId("filter-search"), "L0025");
    const table = screen.getByTestId("queue-table");
    expect(within(table).getByText("L0025")).toBeInTheDocument();
    expect(within(table).queryByText("L0029")).not.toBeInTheDocument();
  });
  ...
  it("행 클릭 시 Drawer가 열리고 닫힌다", async () => {
    ...
    expect(drawer).toHaveTextContent("L0025");
